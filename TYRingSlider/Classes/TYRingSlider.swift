 ...
 * TYRingSlider uses the target-action mechanism to report changes made during the course of editing:
 * ValueChanged, EditingDidBegin and EditingDidEnd
 */
@IBDesignable
open class TYRingSlider: UIControl {
    ...
    @IBInspectable
    open var diskImage: UIImage?
    
    open var diskImageOffset: CGFloat = 0.0
    ...
    open var step: CGFloat?
    
    open var stepTickColor: UIColor?
    
    open var stepTickWidth: CGFloat?
    
    open var stepTickLength: CGFloat?
    ...
    */
    ...
    override open func draw(_ rect: CGRect) {
        ...
        let endAngle = TYRingSliderHelper.scaleToAngle(value: endPointValue, inInterval: valuesInterval) + TYRingSliderHelper.circleInitialAngle
        
        if let _diskImage = diskImage {
            drawDiskImage(withImage: _diskImage, inContext: context)
        } else {
            drawFilledArc(fromAngle: TYRingSliderHelper.circleInitialAngle, toAngle: endAngle, inContext: context)
        }
        ...
        drawThumbAt(endAngle, with: endThumbImage, inContext: context, followAngle: true)
    }
    ...
    override open func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        var value = 0.0
        if let _step = step {
            value = round(newValue(from: endPointValue, touch: touchPosition, start: startPoint) /  _step) * _step
        } else {
            value = newValue(from: endPointValue, touch: touchPosition, start: startPoint)
        }
        endPointValue = value
    ...
    internal func newValue(from oldValue: CGFloat, touch touchPosition: CGPoint, start startPosition: CGPoint) -> CGFloat {
        let angle = TYRingSliderHelper.angle(betweenFirstPoint: startPosition, secondPoint: touchPosition, inCircleWithCenter: bounds.center)
        let interval = Interval(min: minimumValue, max: maximumValue, rounds: numberOfRounds)
        let deltaValue = TYRingSliderHelper.delta(in: interval, for: angle, oldValue: oldValue)
