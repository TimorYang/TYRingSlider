...
open class TYMultiRingPointList: NSObject {
    private(set) var head: TYMultiRingPoint?
    ...
    // 从指定节点开始遍历链表
    func traverse(from node: TYMultiRingPoint? = nil, _ body: (TYMultiRingPoint) -> Bool) {
    ...
    func removeNode(withValue value: CGFloat) {
        ...
        var currentNode: TYMultiRingPoint? = startNode
        var previousNode: TYMultiRingPoint? = nil
    ...
    func remove(node: TYMultiRingPoint) {
        // 如果链表为空或者节点是nil，直接返回
    ...
    func findFirstNode() -> TYMultiRingPoint? {
    ...
    func findEndNode() -> TYMultiRingPoint? {
        guard let startNode = head else { return nil }
        var currentNode: TYMultiRingPoint? = startNode
        repeat {
